...
import net.koreate.project.dao.QnaBoardDAO;
...
public class QnaServiceImpl implements QnaService {
	...
	public void registReply(RegistDTO regist) throws Exception {
		regist.setPassword(password.encode(regist.getPassword()));
		dao.register(regist);
		dao.updateOrigin();
	...
	@Override
	public boolean checkPw(int bno, String pw) {
		boolean check = false;
		System.out.println("pw : "+pw);
		String nowPw = dao.pwCheck(bno);
		System.out.println(nowPw);
		if(password.matches(pw, nowPw)) {
			check = true;
		}
		return check;
	...
	public void replyRegister(BoardVO vo) throws Exception {
		vo.setPassword(password.encode(vo.getPassword()));
		dao.updateReply(vo);
